...
const unsubscribers = [];
export const MessageDashboardUnaccessable = ({ setChannels, subChannels, getSelectedChannelId }) => {
  unsubscribers.forEach(unsub => unsub());

  const mounpoint = document.getElementById('message-dashboard-mountpoint');
  const dashboardWarning = document.getElementById('message-dashboard-unaccessable-component').content.cloneNode(true);
  ...
  joinBtn.addEventListener('click', () => {
    joinChannel(getSelectedChannelId())
      .then(() => {
