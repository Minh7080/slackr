...
export const MessageImage = ({ src, messageId }) => {
  const messageDocument = document.getElementById('message-image-component').content.cloneNode(true);
  ...
  messageElement.addEventListener('click', () => {
    ...
    const currentIndex = images.findIndex(img => img.src.trim() === src.trim() && parseInt(img.getAttribute('message-id')) === messageId);
    
    ImagePreview({ images: images.length > 0 ? images.map(img => img.src) : [src], currentIndex: currentIndex >= 0 ? currentIndex : 0 });
  });
